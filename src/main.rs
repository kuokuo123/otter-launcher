...
impl Highlighter for OtterHelper {
    fn highlight_hint<'h>(&self, hint: &'h str) -> Cow<'h, str> {
        ...
        if cached_statics(&SUGGESTION_MODE, "list".to_string()) == "hint" {
            return format!("{}{}{}{}", "\x1b[0m", hint_color, hint, "\x1b[0m").into()
        } else {
                ...
                .map(|line| {
                    if line == place_holder {
                        format!("{}{}{}", 
                            ...
                            place_holder, 
                            "\x1b[0m")
                    } else if empty_module_message.contains(line) && !line.is_empty() {
                        line.to_string()
                    } else if default_module_message.contains(line) && !line.is_empty() {
                        line.to_string()
                    } else {
...
fn init_statics<T: Clone>(lazy_value: &Lazy<Mutex<Option<T>>>, config_value: Option<T>, default_value: T) {
...
fn main() {
    ...
    init_statics(&CHEATSHEET_ENTRY, CONFIG.general.cheatsheet_entry.clone(), "?".to_string());
    init_statics(&CHEATSHEET_VIEWER, CONFIG.general.cheatsheet_viewer.clone(), "less -R".to_string());
    init_statics(&VI_MODE, CONFIG.general.vi_mode.clone(), false);
    ...
    init_statics(&LIST_PREFIX, CONFIG.interface.list_prefix.clone(), "".to_string());
    init_statics(&PLACE_HOLDER, CONFIG.interface.place_holder.clone(), "type and search...".to_string());
    init_statics(&INDICATOR_WITH_ARG_MODULE, CONFIG.interface.indicator_with_arg_module.clone(), "".to_string());
    init_statics(&INDICATOR_NO_ARG_MODULE, CONFIG.interface.indicator_no_arg_module.clone(), "".to_string());
    init_statics(&SUGGESTION_MODE, CONFIG.interface.suggestion_mode.clone(), "list".to_string());
    init_statics(&SUGGESTION_LINES, CONFIG.interface.suggestion_lines.clone(), 1);
    init_statics(&SUGGESTION_SPACING, CONFIG.interface.suggestion_spacing.clone(), 0);
    init_statics(&DEFAULT_MODULE_MESSAGE, CONFIG.interface.default_module_message.clone(), "".to_string());
    init_statics(&EMPTY_MODULE_MESSAGE, CONFIG.interface.empty_module_message.clone(), "".to_string());
    init_statics(&PREFIX_PADDING, CONFIG.interface.prefix_padding.clone(), 0);
    init_statics(&PREFIX_COLOR, CONFIG.interface.prefix_color.clone(), "".to_string());
    init_statics(&DESCRIPTION_COLOR, CONFIG.interface.description_color.clone(), "".to_string());
    init_statics(&PLACE_HOLDER_COLOR, CONFIG.interface.place_holder_color.clone(), "\x1b[30m".to_string());
    init_statics(&HINT_COLOR, CONFIG.interface.hint_color.clone(), "\x1b[30m".to_string());
    ...
    if cached_statics(&VI_MODE, false) { rl.set_edit_mode(EditMode::Vi) };
    ...
    match module_prfx {
        ...
        None => {
            ...
            if prompt.is_empty() {
                run_designated_module(
                    prompt,
                    cached_statics(&EMPTY_MODULE, "".to_string()))
